...
class CvtSelfAttention(nn.Module):
    ...
    def forward(self, hidden_state, encoder_hidden_state, height, width,
                encoder_height, encoder_width):
        r"""
        hidden_state: query_embed, (1, num_queries, embed_dim), num_queries = h * w
        encoder_hidden_state: image_features, (batch_size, eh * ew + 1, embed_dim)
        return:
        (1, num_queries, embed_dim)
        """
        if self.with_cls_token:
            cls_token, encoder_hidden_state = torch.split(encoder_hidden_state, [1, encoder_height * encoder_width], 1)
        batch_size, hidden_size, num_channels = hidden_state.shape
        # rearrange "b (h w) c -> b c h w"
        hidden_state = hidden_state.permute(0, 2, 1).view(batch_size, num_channels, height, width)
        encoder_hidden_state = encoder_hidden_state.permute(0, 2, 1).view(batch_size, num_channels, encoder_height, encoder_width)

        key = self.convolution_projection_key(encoder_hidden_state)
        query = self.convolution_projection_query(hidden_state)
        value = self.convolution_projection_value(encoder_hidden_state)
...
class CvtAttention(nn.Module):
    ...
    def forward(self, hidden_state, encoder_hidden_states, height, width,
                encoder_height, encoder_width):
        "hidden_state shape: (b, h*w, c)"
        self_output = self.attention(hidden_state, encoder_hidden_states, height, width, encoder_height, encoder_width)
...
class BertLayer(nn.Module):
    def __init__(self, config, layer_num):
        ...
        if (
            ...
            self.crossattention = CvtAttention(
                config.num_heads,
                config.embed_dim,
                config.kernel_size,
                config.padding_q,
    ...
    def forward(
        ...
        if query_length > 0:
            ...
            if self.has_cross_attention:
                ...
                # encoder_hidden_states is image feature, shape of (b, l, d)
                # query_length must be squares
                height = weight = int(math.sqrt(query_length))
                encoder_height = encoder_width = int(math.sqrt(encoder_hidden_states.size(1)))
...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        # print(input_ids.size(), attention_mask.size() if attention_mask is not None else attention_mask)
        # (6, 1), (6, 1)
        # print(query_embeds.size())
        if attention_mask is None:
            attention_mask = input_ids.new_ones(input_ids.shape)
        query_mask = input_ids.new_ones(query_embeds.shape[:-1])
        # print(query_mask.size(), attention_mask.size())  (18, 25), (6, 1)
